...
def generate_Permutations(depth,temp,f):
	...
	if(depth < 6):
		...
		for i in l:
			...
			if t_s_hash == "nd4.kYa98cdFa3X85Ho8j1":
				print(t_s)
				return t_s
			generate_Permutations(depth + 1, t_s,f)
	

def gp(depth,temp,li):
	l = "abcdefghijklmnopqrstuvwxyz"
	if(depth < 7):
		for i in l:
			t_s = temp
			t_s += i
			li.append((t_s,"nothashed"))
			print(t_s)
			gp(depth + 1, t_s,li)
...
def compute_hash(pw):
	#4fTgjp6q$nd4.kYa98cdFa3X85Ho8j1
	pw.encode('utf-8')
...
print(generate_Permutations(0,"",f))
"""
def Compute_Hash(pw):
